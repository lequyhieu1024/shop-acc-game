...
export const POST = async (req: NextRequest): Promise<NextResponse> => {
    try {
        ...
        const imgUrl: string | NextResponse = await uploadFileToPinata(newData.image_url);

        const newBanner = bannerRepo.create({ is_active: newData.is_active, image_url: imgUrl })
