...
export const POST = async (req: NextRequest) => {
    try {
        ...
        if (!isInteger(data.code)) {
            return NextResponse.json({result: false, message: "Mã thẻ phải là số!", status: 400 });
        }

        if (!isInteger(data.serial)) {
            return NextResponse.json({result: false, message: "Số seri phải là số!", status: 400 });
        }
        ...
        if (!rule) {
            return NextResponse.json({result: false, message: "Nhà mạng không hợp lệ!", status: 400 });
        }
        if (data.code.length !== rule.code) {
            return NextResponse.json({result: false, message: `Mã thẻ của ${data.telco} phải có ${rule.code} ký tự!`, status: 400 });
        }

        if (rule.serial && data.serial.length !== rule.serial) {
            return NextResponse.json({result: false, message: `Serial của ${data.telco} phải có ${rule.serial} ký tự!`, status: 400 });
        }
