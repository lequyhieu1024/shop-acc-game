...
import {Carousel, Rate,Modal} from "antd";
import {
    // DeleteOutlined,
    // UserOutlined,
    // ShopOutlined,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <Carousel autoplay>
                        ...
                        {/*    product!.images*/}
                        {/*        .filter((img): img is IProductImage => "image_url" in img)*/}
                        {/*        .map((img, index) => (*/}
                        {/*            <img*/}
                        {/*                key={index}*/}
                        ...
                        {Array.isArray(product.images) && product.images.length > 0 ? (
                            (product.images as IProductImage[]).map((img, index: number) => (
                                <img
                                    key={index}
                                    src={img.image_url || "/client/assets/images/placeholder.png"}
                                    alt={product.name}
                ...
                <div>
                    ...
                    <div className="flex items-center mt-2">
                        ...
                        <span className="ml-2 text-gray-500">5 đánh giá</span>
                    </div>
                    <span className="font-semibold">
                        <del>{product.regular_price}đ { ' ' }</del>
                        <span className={'text-xl text-red-500'}>{product.sale_price} đ</span>
                    </span>
                    <p className="text-gray-500">Số lượng còn: {product.quantity}</p>
                    <p className="text-gray-500">Loại đăng ký: {product.register_by}</p>
                    {
                        product.is_infinity_card &&
                        <p className="text-gray-500">Thẻ vô cực: {product.is_infinity_card ? "có" : "không"}</p>
                    }
                    {
                        product.server && <p className="text-gray-500">Server: {product.server}</p>
                    }
                    {
                        product.skin_type && <p className="text-gray-500">Loại skin: {product.skin_type}</p>
                    }
                    {
                        product.rank && <p className="text-gray-500">Rank: {product.rank}</p>
                    }
                    {
                        product.number_diamond_available !== 0 &&
                        <p className="text-gray-500">SL kim cương đang có: {product.number_diamond_available}</p>
                    }
                ...
                <ShoppingCartOutlined className="mr-1"/> Thêm vào giỏ hàng
                        ...
                        <button
                            className="w-full bg-pink-100 text-pink-500 py-2 px-4 rounded-md flex items-center justify-center text-sm hover:text-white hover:bg-pink-500 transition">
                            Mua ngay
                        </button>
                    </div>
                </div>
